...
public class ProcessOnMove {
  ...
  public static void processOnMove(
      MoveRequest moveMessage, MoveResponse response, boolean goToNextPlayer) {
    //        // only don't go to next player when playing a SPLIT card, since you have to make
    // processOnMove twice
    ...
    //                return;
    //            }
    //            if(moveMessage.getPawn2() != null &&
    // !Objects.equals(moveMessage.getPawn2().getPlayerId(), playerId)){
    //                response.setResult(CANNOT_MAKE_MOVE);
    //                return;
    //            }
    //        }
    ...
    //            Log.info("GameState: OnMove: normal route between 0,15 but could move to next
    // section");
    //            // check
    //
    //            if(currentTileId.getTileNr() < 1){moves.add(new
    // PositionKey(currentTileId.getPlayerId(), 1));}
    //            if(currentTileId.getTileNr() < 7){moves.add(new
    // PositionKey(currentTileId.getPlayerId(), 7));}
    //            if(currentTileId.getTileNr() < 13){moves.add(new
    // PositionKey(currentTileId.getPlayerId(), 13));}
    //            if(currentTileId.getTileNr() < 15){moves.add(new
    // PositionKey(currentTileId.getPlayerId(), 15));}
    //
    //            startTileId = new PositionKey(nextPlayerId(playerIdOfTile), 0);
    //            if (canPassStartTile(pawn1, startTileId)){
    //                Log.info("GameState: OnMove: can move past StartTile "+new
    // PositionKey(playerIdOfTile+1,0));
    ...
    //            }else { // or turn back
    //                Log.info("GameState: OnMove: normal route is blocked by a start tile, move
    // backwards");
    //                next = 15 - next%15;
    ...
    //                processMove(pawn1, new PositionKey(playerIdOfTile,next), moveMessage,
    // response, goToNextPlayer);
    //            }else{
    //                response.setResult(CANNOT_MAKE_MOVE);
    //            }
    //            return;
    ...
    //                }
    //            }else{
    //                if(next < 13 && currentTileId.getTileNr() > 13){moves.add(new
    // PositionKey(playerIdOfTile, 13));}
    //                if(next < 7 && currentTileId.getTileNr() > 7){moves.add(new
    // PositionKey(playerIdOfTile, 7));}
    //                if(next < 1 && currentTileId.getTileNr() > 1){moves.add(new
    // PositionKey(playerIdOfTile, 1));}
    //            }
    //
    //            moves.add(nextTileId);
    //            response.setMovePawn1(moves);
    ...
    //                Log.info("GameState: OnMove: pawn wants to go backwards but is blocked by a
    // start tile, goes forwards");
    //                next = -next+2; // +1 : you can't move on tile 0 and would then move on tile 1
    // twice.
    //            }
    //
    //            PositionKey nextTileId = new PositionKey(playerIdOfTile, next);
    ...
    //            if(!tileIsABlockade(new PositionKey(playerIdOfTile,0)) &&
    // cannotMoveToTileBecauseSamePlayer(pawn1, new PositionKey(playerIdOfTile,0))){
    //                response.setResult(CANNOT_MAKE_MOVE);
    //                return;
    //            }
    ...
    //                ArrayList<PositionKey> pingpongmoves = pingpongMove(pawn1, currentTileId,
    // nrSteps);
    //                moves.clear();
    //                moves.addAll(pingpongmoves);// todo is this necessary?
    //                response.setMovePawn1(moves);
    //                processMove(pawn1, pingpongmoves.getLast(), moveMessage, response,
    // goToNextPlayer);
    //                return;
    //            }
    //
    ...
    //            if(nrSteps > 0){
    //                tileHighestTileNr = checkHighestTileNrYouCanMoveTo(pawn1, currentTileId,
    // nrSteps);
    ...
    //                }
    //            }
    //            if (targetTileId.getTileNr() < 15) {moves.add(new
    // PositionKey(targetTileId.getPlayerId(), 15));}
    //            if (targetTileId.getTileNr() < 13) {moves.add(new
    // PositionKey(targetTileId.getPlayerId(), 13));}
    //            if (targetTileId.getTileNr() < 7) {moves.add(new
    // PositionKey(targetTileId.getPlayerId(), 7));}
    //
    ...
    //            PositionKey targetTileId = moveAndCheckEveryTile(pawn1, currentTileId, nrSteps);
    //
    //            int tileHighestTileNr = checkHighestTileNrYouCanMoveTo(pawn1, currentTileId,
    // nrSteps);
    //            if(tileHighestTileNr > targetTileId.getTileNr()){
    //                if(tileHighestTileNr > 15){
    //                    // move to finish
    //                    moves.add(new PositionKey(nextPlayerId(playerIdOfTile),
    // tileHighestTileNr));
    //                    // possibly move back out of finish
    //                    // otherwise if 16 was taken, then it would move (0,15) (1,15) (0,15) and
    // then correctly back
    //                    if(targetTileId.getTileNr() < 15){moves.add(new
    // PositionKey(targetTileId.getPlayerId(), 15));}
    //                }
  ...
  //        return false;
  //      }
  //      if (Objects.equals(pawn.getPlayerId(), nextTileId.getPlayerId()) && nextTileId.getTileNr()
  // == 0){
  //        return false;
  //      }
  //    }
  ...
  //  }
  //
  //  private boolean cannotMoveToTileBecauseSamePlayer(PawnId selectedPawnId, PositionKey
  // nextTileId){
  //    Pawn pawn = getPawn(nextTileId);
  //    if(pawn != null) {
  ...
  //
  //    // an extra check to see if the first two moves are identical. this can happen when you do
  // -4 steps and are

...
//        }
        ...
        if(!isJack(card)){
            if(moveMessage.getPawn1() != null && !Objects.equals(moveMessage.getPawn1().getPlayerId(), playerId)){
                response.setResult(CANNOT_MAKE_MOVE);
                return;
            }
...
//        }
      //todo: do not uncomment above
        ...
        PositionKey tileId1 = new PositionKey(pawn1.getCurrentTileId().getPlayerId(), pawn1.getCurrentTileId().getTileNr());
        PositionKey tileId2 = new PositionKey(pawn2.getCurrentTileId().getPlayerId(), pawn2.getCurrentTileId().getTileNr());
        // switch in gamestate
    ...
    public void processMove(Pawn pawn, PositionKey targetTileId, MoveRequest moveMessage, MoveResponse response){
        processMove(pawn, targetTileId, moveMessage, response, true);
    }

    public void processMove(Pawn pawn0, PositionKey targetTileId, MoveRequest moveMessage, MoveResponse response, boolean goToNextPlayer){
        String playerId = moveMessage.getPlayerId();
        Card card = moveMessage.getCard();

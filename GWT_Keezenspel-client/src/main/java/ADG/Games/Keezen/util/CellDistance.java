...
public class CellDistance {
  ...
  public static double getCellDistance(int nrPlayers, double boardSize) {
    ...
    // this distance is dependent on the number of players: 2 -> x=0 so that two boards are directly
    // joined
  ...
  public static Point getStartPoint(int nrPlayers, double boardSize) {
    ...
    Point startPoint =
        new Point(
            midPoint.getX() + 2 * getCellDistance(nrPlayers, boardSize),
            midPoint.getY() + y * getCellDistance(nrPlayers, boardSize));
    // midPoint.getY() + y);
    GWT.log("y value = " + y);
